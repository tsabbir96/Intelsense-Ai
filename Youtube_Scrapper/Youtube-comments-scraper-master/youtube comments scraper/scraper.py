...
def _find_comments(browser):
    ...
    comment_dates = browser.find_elements_by_xpath('//*[@id="header-author"]/yt-formatted-string')
  
    comments = []
    for author_name, comment, date in zip(author_names, comment_texts,comment_dates):
        a_comment = {
...
def run_scraper(urls, numOfScrolls=30):
    allData = []
    for url in urls:
        try:
            browser.get(url)
